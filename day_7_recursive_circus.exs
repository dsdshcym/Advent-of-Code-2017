defmodule Tower do
  ...
  defp parse_line(line) do
    ...
    |> (fn
          ...
          [node_name, _weight, "->" | descendants] ->
            {node_name, descendants}
        end).()
  end

  defp do_find_bottom(node_and_descendants) do
    {all_nodes, all_descendants} = Enum.unzip(node_and_descendants)
    all_nodes = all_nodes |> MapSet.new()
    all_descendants = all_descendants |> List.flatten() |> MapSet.new()

    MapSet.difference(all_nodes, all_descendants)
    |> MapSet.to_list()
    |> hd()
  end
end
...
defmodule TowerTest do
  ...
  describe "find_bottom/1" do
    test "only one program" do
      assert Tower.find_bottom("abcd (11)") == "abcd"
    end

    test "only two programs" do
      assert Tower.find_bottom("ef (22) -> abcd") == "ef"
    end

    test "returns the program who has no program beneath it" do
      assert Tower.find_bottom("""
             ef (22) -> abcd
             abcd (11)
             """) == "ef"
    end
